...
MODEL_VERSION = "香港六合彩預測系統_20260707_第26版"
...
_NUMBER_BACKTEST_PROFILE_CACHE: dict[tuple[int, str, str, int, int, str], dict[int, dict[str, int]]] = {}
...
def calculate_rolling_direction_scores(draws: list[Draw], recent_window: int) -> dict[int, float]:
    if not draws:
        return {number: 0.5 for number in range(MIN_NUMBER, MAX_NUMBER + 1)}
    window_size = max(12, min(recent_window, 36, len(draws)))
    short_size = max(4, min(8, len(draws)))
    recent_draws = draws[-window_size:]
    short_draws = draws[-short_size:]
    recent_counts = Counter(number for draw in recent_draws for number in draw.main_numbers)
    short_counts = Counter(number for draw in short_draws for number in draw.main_numbers)
    main_history = [draw.main_numbers for draw in draws]
    gaps = calculate_miss_gaps(main_history)
    pair_strength = calculate_pair_strength(main_history)
    max_recent = max(recent_counts.values(), default=1) or 1
    max_short = max(short_counts.values(), default=1) or 1
    max_pair = max(pair_strength.values(), default=1.0) or 1.0
    raw: dict[int, float] = {}
    for number in range(MIN_NUMBER, MAX_NUMBER + 1):
        gap = gaps[number]
        recent_heat = recent_counts[number] / max_recent
        short_heat = short_counts[number] / max_short
        gap_sweet_spot = clamp01(1.0 - abs(gap - 5) / 12.0)
        pair_support = pair_strength[number] / max_pair
        just_opened_penalty = 0.16 if gap == 0 else 0.08 if gap == 1 else 0.0
        long_cold_penalty = 0.10 if gap >= 28 and recent_counts[number] == 0 else 0.0
        raw[number] = clamp01(
            recent_heat * 0.38
            + short_heat * 0.18
            + gap_sweet_spot * 0.22
            + pair_support * 0.14
            + (0.08 if 2 <= gap <= 12 else 0.0)
            - just_opened_penalty
            - long_cold_penalty
        )
    return normalized_values(raw)
...
def build_auto_scores(
    ...
    rolling_direction_scores = calculate_rolling_direction_scores(draws, recent_window)
    for number in range(MIN_NUMBER, MAX_NUMBER + 1):
        normalized_parts = []
        raw_parts = []
        for strategy, score_map in strategy_score_maps.items():
        ...
        model_scores["rolling_direction"] = rolling_direction_scores[number]
        adjusted_score = (
            maturity_score * 0.66
            + validation_scores[number] * WALK_FORWARD_VALIDATION_WEIGHT
            + raw_score * 0.06
            + rolling_direction_scores[number] * 0.10
...
def generate_auto_tickets(
    ...
    for strategy, generated in generated_by_strategy.items():
        ...
        for rank, ticket in enumerate(ranked_strategy_tickets, start=1):
            ...
            rolling_direction = statistics.mean(rolling_direction_scores[number] for number in ticket.numbers)
            adjusted_score = (
                strategy_weight * 1.15
                + normalized_score * 0.85
                + maturity * 0.42
                + validation * STRICT_TICKET_VALIDATION_WEIGHT
                + rolling_direction * 0.22
...
def confidence_ticket_status(package: PredictionPackage, limit: int = 6) -> tuple[str, str]:
    ...
    if observed:
        return "未達第26版高機率門檻", f"{observed[1]} {observed[2]}，只列觀察"
    return "未達第26版高機率門檻", "無可列候選"
        ...
        if missed:
            rows.append(
                [
                    ...
                    "第26版結算記憶 + 過曝控管 + 轉移追蹤會重新校正漏抓號、鄰近號、同尾號、同區間號",
                ]
            )
        ...
        if mid_hits >= 4:
            rows.append(
                [
                    ...
                    "第26版區間修復 + 回測驗證提高 11-30 中段與同尾橋接權重",
                ]
            )
    ...
    if missing_hot:
        rows.append(
            [
                ...
                "第26版本月滾動 + 日曆相位 + 回測驗證共同前移，不再只當防守補位",
            ]
        )
    if not rows:
        rows.append(
...
def build_battle_report_markdown(conn: sqlite3.Connection, recent_window: int) -> str:
    ...
    super_rows_preview = super_recommendation_rows(package, draws)
    super_preview_text = "；".join(f"{row[0]} {row[1]}" for row in super_rows_preview[:3]) or "未達強推門檻"
    avoid_preview_text = "；".join(f"{row[0]} {row[1]} {row[2]}" for row in avoid_rows_preview[:3]) or "暫無低機率包"
    latest_numbers_text = f"{format_numbers(latest.main_numbers)} + 特別號 {latest.special:02d}"
    ...
    lines: list[str] = [
        ...
        markdown_table(
            ["項目", "內容"],
            [
                ["資料依據日", based_draw_date],
                ["資料依據期", based_draw_no or "-"],
                ["預測目標日", target_date],
                ["最新開獎", f"{latest.draw_no or '-'} / {latest.draw_date} / {latest_numbers_text}"],
                ["產生時間", report_time],
                ["模型版本", MODEL_VERSION],
                ["歷史資料範圍", f"{len(draws)} 期 / {date_range}"],
            ],
        ),
        "",
        f"## 核心決策（資料依據日 {based_draw_date} / 預測目標日 {target_date}）",
        markdown_table(
            ["項目", "內容", "驗算位置"],
            [
                ["強推薦", super_preview_text, "最強獨隻1中1、超強信心高機率強推薦號碼"],
                ["前九核心", format_numbers(top9), "下期研究候選前9名、生成號碼逐號驗算"],
                ["高機率信心牌", f"{confidence_status_preview}：{confidence_text_preview}", "高機率信心牌、推薦門檻檢查"],
                ["低機率暫避", avoid_preview_text, "低機率、暫避號碼逐號驗算"],
                ["發布等級", f"{release_level} / 風險{risk_level}", "模型回測、研究命中指標"],
                ["資料完整度", f"{completeness_passed}/{completeness_total}", "其他稽核、資料完整度總表"],
            ],
        ),
        "",
        "## 最強獨隻1中1",
        markdown_table(
            ...
            super_rows_preview[:1] or [["未達門檻", "-", "-", "-", "-", "本期不強行發布"]],
        ),
        "",
        f"## 下期研究候選前9名（資料依據日 {based_draw_date} / 預測目標日 {target_date}）",
        markdown_table(
            ...
            [
                [
                    ...
                    row.miss_gap,
                    model_pass_count_text(row),
                    top_model_text(row, 4),
                ]
                for rank, row in enumerate(ranked_scores[:CORE_POOL_SIZE], start=1)
            ],
        ),
        "",
        f"## 生成號碼逐號驗算（資料依據日 {based_draw_date} / 預測目標日 {target_date}）",
        markdown_table(
            [
                "號碼",
                "資料依據日",
            ...
            generated_number_verification_rows(
                package,
                ranked_numbers,
                score_max,
                draws,
        ...
        markdown_table(
            ["項目", "內容", "位置"],
            [
                ["強推薦", super_preview_text, "強推薦逐號驗算"],
        ...
        markdown_table(
            ...
            [
                ...
                ["新一期結構", f"前九={format_numbers(top9)}", "符合第26版每期重算、539鐵律與九顆核心池規格"],
            ],
        ),
        "",
        "## 其他稽核：全系統缺口檢測與第26版修復",
    ...
    lines.extend(
        [
            ...
            "",
            f"## 模型回測：研究命中指標與禁止虛報門檻（樣本 {rank_backtest.get('sample', 0)} 期）",
            markdown_table(
...
def number_backtest_profiles(
    ...
    if len(draws) < 20 or not selected:
        return {number: dict(blank_profile) for number in selected}

    latest = draws[-1]
    cache_key = (len(draws), latest.draw_date, latest.draw_no, recent_window, max_periods, strategy)
    cached = _NUMBER_BACKTEST_PROFILE_CACHE.get(cache_key)
    if cached is not None:
        return {number: dict(cached.get(number, blank_profile)) for number in selected}
    ...
    _NUMBER_BACKTEST_PROFILE_CACHE[cache_key] = profiles
    return {number: dict(profiles.get(number, blank_profile)) for number in selected}
...
def model_pass_count_text(row: NumberScore, threshold: float = 0.56) -> str:
    total = len(row.model_scores)
    passed = sum(1 for value in row.model_scores.values() if value >= threshold)
    return f"{passed}/{total}"


def generated_number_verification_rows(
    package: PredictionPackage,
    ranked_numbers: list[int],
    score_max: float,
    draws: list[Draw],
    ...
    super_numbers = [number for item in super_recommendation_items(package, draws) for number in item["numbers"]]
    numbers = list(dict.fromkeys([*ranked_numbers[:CORE_POOL_SIZE], *super_numbers]))
    profiles = number_backtest_profiles(draws, recent_window, numbers)
    rows = []
    for number in numbers:
        ...
        layer_checks = [
            rank <= CORE_POOL_SIZE,
            sum(1 for value in row.model_scores.values() if value >= 0.56) >= 4,
            any(number in ticket.numbers for ticket in package.tickets),
            row.recent_frequency > 0 or row.miss_gap >= 6,
            profiles[number].get("core_seen", 0) > 0 or profiles[number].get("support_seen", 0) > 0,
            number not in package.weak_numbers,
        ]
        rows.append(
            [
                f"{number:02d}",
                ...
                f"{row.score:.3f}",
                confidence_label(row, score_max),
                f"{sum(1 for item in layer_checks if item)}/{len(layer_checks)}",
                f"{number_source_tags(number, package, ranked_numbers)}；{ticket_source_text(number, package)}",
                drag_cross_text(number, package, row),
                f"{wave_color(number)}、尾{number % 10}、{decade_bucket(number)}區、遺漏{row.miss_gap}期、近窗{row.recent_frequency}次",
                top_model_text(row, 3),
                number_backtest_text(profiles[number]),
                (
                    f"{model_label('failure_cooldown')}{row.model_scores.get('failure_cooldown', 0.0):.2f}、"
                    f"{model_label('settlement_memory')}{row.model_scores.get('settlement_memory', 0.0):.2f}、"
                    f"{model_label('overexposure_guard')}{row.model_scores.get('overexposure_guard', 0.0):.2f}"
                ),
                number_validation_decision(number, rank, package, model_text),
            ]
        )
    ...
    for number_text, values in snapshot.items():
        ...
        try:
            ...
            rows.append(
                NumberScore(
                    number=number,
                    total_frequency=int(values.get("total_frequency", 0) or 0),
                    recent_frequency=int(values.get("recent_frequency", 0) or 0),
                    special_frequency=int(values.get("special_frequency", 0) or 0),
                    miss_gap=int(values.get("miss_gap", 0) or 0),
                    trend=float(values.get("trend", 0.0) or 0.0),
                    pair_strength=float(values.get("pair_strength", 0.0) or 0.0),
                    score=float(values.get("score", 0.0) or 0.0),
                    color=str(values.get("color", "")),
                    model_scores={
                        str(name): float(value)
                        for name, value in (values.get("model_scores") or {}).items()
                    },
                )
            )
        except (TypeError, ValueError):
            continue
    score_max = max((row.score for row in rows), default=1.0) or 1.0
    ranked_numbers = [row.number for row in sorted(rows, key=lambda row: row.score, reverse=True)]
    rank_map = {number: index for index, number in enumerate(ranked_numbers, start=1)}
    low_rows = sorted(
        rows,
        key=lambda row: low_probability_priority(row, score_max, rank_map.get(row.number, MAX_NUMBER)),
        reverse=True,
    )
    return [row.number for row in low_rows[:count]]
...
def low_probability_recent_open_penalty(row: NumberScore) -> float:
    if row.miss_gap <= 0:
        return 0.55
    if row.miss_gap == 1:
        return 0.42
...
def low_probability_hot_penalty(row: NumberScore) -> float:
    hot_penalty = clamp01((row.recent_frequency - 3) / 6) * 0.28
    memory_penalty = row.model_scores.get("settlement_memory", 0.0) * 0.14
    direction_penalty = row.model_scores.get("rolling_direction", 0.0) * 0.12
    return hot_penalty + memory_penalty + direction_penalty


def low_probability_priority(row: NumberScore, score_max: float, rank: int) -> float:
    score_max = score_max or 1.0
    low_score = clamp01(1.0 - row.score / score_max)
    rank_pressure = clamp01((rank - CORE_POOL_SIZE) / (MAX_NUMBER - CORE_POOL_SIZE))
    recent_cold = clamp01((3 - row.recent_frequency) / 3)
    miss_safety = clamp01((row.miss_gap - 3) / 12)
    pair_weak = clamp01(1.0 - row.model_scores.get("cooccurrence_graph", 0.0))
    trend_weak = clamp01(0.55 - normalize_signed(row.trend))
    priority = (
        low_score * 0.30
        + rank_pressure * 0.18
        + miss_safety * 0.24
        ...
        + pair_weak * 0.07
        + trend_weak * 0.05
        - low_probability_recent_open_penalty(row)
        - low_probability_hot_penalty(row)
    )
    return priority
...
def low_probability_reason(row: NumberScore) -> str:
    reasons = []
    if row.miss_gap <= 2:
        reasons.append("近期剛開，暫避降權")
    elif row.miss_gap >= 6:
        reasons.append("遺漏冷卻足夠")
    if row.recent_frequency == 0:
        reasons.append("近期未形成熱度")
    elif row.recent_frequency >= 4:
        reasons.append("短窗偏熱，暫避降權")
    if row.pair_strength <= 0:
...
def exclusion_confidence(row: NumberScore, score_max: float, rank: int) -> float:
    ...
    miss_safety = clamp01((row.miss_gap - 3) / 12)
    recent_open_penalty = low_probability_recent_open_penalty(row) * 38
    hot_penalty = low_probability_hot_penalty(row) * 32
    value = (
        low_score * 34
        + rank_pressure * 16
        + recent_cold * 14
...
def exclusion_backtest_summary(
    ...
    for index in range(start, len(draws)):
        ...
        ranked_numbers = [row.number for row in sorted(scores.values(), key=lambda row: row.score, reverse=True)]
        rank_map = {number: rank for rank, number in enumerate(ranked_numbers, start=1)}
        low_rows = sorted(
            scores.values(),
            key=lambda row: low_probability_priority(row, score_max, rank_map.get(row.number, MAX_NUMBER)),
            reverse=True,
...
REPORT_TAB_NAMES = ("下期預測", "命中檢討", "每月總整理", "低機率", "模型回測", "其他稽核")
...
def build_report_tabs_html() -> str:
    buttons = "\n".join(
        f'      <button type="button" class="report-tab{" active" if name == "下期預測" else ""}" data-tab-button="{name}">{name}</button>'
...
<head>
  ...
  <style>
    ...
    .overview-band h2::before {{ content: "預測"; background: #2563eb; }}
    .prediction-band h2::before {{ content: "預測"; background: #b42318; }}
    .control-band h2::before {{ content: "檢討"; background: #15803d; }}
    .appendix-band h2::before {{ content: "稽核"; background: #64748b; }}
    .band[data-tab="下期預測"] h2::before {{ content: "下期預測"; background: #b42318; min-width: 72px; }}
    .band[data-tab="命中檢討"] h2::before {{ content: "命中檢討"; background: #15803d; min-width: 72px; }}
    .band[data-tab="每月總整理"] h2::before {{ content: "每月總整理"; background: #0f766e; min-width: 92px; }}
    .band[data-tab="低機率"] h2::before {{ content: "低機率"; background: #7c3aed; min-width: 64px; }}
    .band[data-tab="模型回測"] h2::before {{ content: "模型回測"; background: #c2410c; min-width: 72px; }}
    .band[data-tab="其他稽核"] h2::before {{ content: "其他稽核"; background: #64748b; min-width: 72px; }}
    .confidence-band {{
...
<body>
  <header>
    <h1>香港六合彩精算預測戰報</h1>
    <div class="meta">539同規格分類分頁戰報 / 自動更新、自動結算、自動預測</div>
  </header>
{tabs_html}
  <main>
{body}
...
def load_mobile_cloud_module():
    ...
    return importlib.import_module("香港六合彩預測系統_手機雲端_20260707_第26版")
